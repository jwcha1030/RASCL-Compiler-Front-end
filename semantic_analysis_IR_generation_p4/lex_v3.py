...
tokens_nonOp={'if':'IF', 'else':'ELSE', 'while':'WHILE','int':'INT', 'float':'FLOAT',
        ...
        'print':'PRINT', 'read':'READ', 'function': 'FUNCTION', 'return':'RETURN'};
__listOfTokens=[]
__listOfCommentTokens=[]
tokenIndex=0
...
class Token():
    ...
    def printTokenInfo(self):
        if self._tokenType in list(tokens_op.values())+list(tokens_relop.values())+ list(tokens_punc.values()):
            print(f'{self._tokenType} [{self._lineNum}:{self._lineIndex}]') #[Line:Index]
        elif self._tokenType == 'ID':
            print(f'ID: {self._tokenStr} [{self._lineNum}:{self._lineIndex}]')
        elif self._tokenType =='ICONST' or self._tokenType == 'FCONST':
            print(f'{self._tokenType}: {self._tokenStr} [{self._lineNum}:{self._lineIndex}]')
            ...
            print(f'{self._tokenType} [{self._lineNum}:{self._lineIndex}]')
        else:
            print(f'Keyword: {self._tokenType} [{self._lineNum}:{self._lineIndex}]')
                ...
                if nextLineMatch:
                    ...
                    if tokenObj:
                        ...
                        if tokenObj._tokenType == 'COMMENT':
                            ...
                            __listOfCommentTokens.append(tokenObj)
                        else:
                        ...
                        print(errorStr)
                        sys.exit(1)
        __listOfTokens.append(Token("$$", "DD", lineNum, xi))
...
def __createAToken(string, lineNum, li, xi):
    ...
    #tokens that are left out (i.e., keywords and ID)
        ...
        if re.match(r'\bif\b',token):
            ...
            return Token(token, tokens_nonOp[token], lineNum, xi),fi
        elif re.match(r'\breturn\b',token):
            return Token(token, tokens_nonOp[token], lineNum, xi),fi
        else:
...
def peekPrevTokenOf(token:Token) -> Token:
    matchIndex=0
    for t in __listOfTokens:
        if token.equals(t):
            if len(__listOfTokens) == 1:
                return None
...
if __name__ == "__main__":
    ...
    print(f"\tList Length: {len(__listOfCommentTokens)}")
    for i in range(len(__listOfCommentTokens)):
        __listOfCommentTokens[i].printTokenInfo()
